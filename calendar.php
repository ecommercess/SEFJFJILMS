...
<section id="calendar">
    ...
    // Fetch event dates with approved status
    $sql = "SELECT event_date FROM bookings WHERE status = 'approved'";
    $result = $conn->query($sql);

    $bookings = [];
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $date = date('Y-m-d', strtotime($row['event_date'])); // Format the date for storage
            if (!isset($bookings[$date])) {
                $bookings[$date] = 0; // Initialize if not set
            }
            $bookings[$date]++; // Increment booking count for the date
        }
    } 
    ...
    // Prepare bookings for JSON output
    $events = [];
    foreach ($bookings as $date => $count) {
        $color = ($count == 1) ? 'green' : 'red'; // Set color based on the count
        $events[] = [
            'start' => $date,
            'end' => $date,
            'rendering' => 'background', // Use background rendering
            'color' => $color
        ];
    }
    ?>
...
overflow: hidden;
background-color: #f8f5f2; /* Light background color */
}
...
justify-content: space-between;
padding: 15px;
background-color: #f9bc60; /* Toolbar background color */
...
.fc-day.fc-today {
background-color: grey !important; /* Fill the full cell with suggested color */

}
.fc-day-number {
font-size: 1.2rem; /* Adjust this size as desired */
font-weight: bold;
color: #333; /* Adjust color if needed */
}

.fc-day.fc-today .fc-day-number {
color: #ffffff; /* Optional: make today's date stand out */
...
$('#calendardate').fullCalendar({
    ...
    eventRender: function(event, element) {
        element.hide().fadeIn(500); // Smooth fade-in for events
    }
});
});
